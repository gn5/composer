...
(def def_vol 60) ;default volume pianoteq
(def def_volbass 60)
           ...
           (if (= end_level new-state) (do
             ...
             (if (not= end_level new-state) (do
                                   (def last_buff (vector (nth all_notes (dec @level)))) ; index start at 0
                                      ...
                                      (def beat_ (metro))
                                        (def ti1 (+ 1 beat_))
                                        (def ti2 (+ legato  (+ 2 beat_)))
                                        (apply-at (metro ti1)    smi2bass [(note :a4)])
                                        (apply-at (metro ti2)    smi2off [(note :a4)])
                                      (doseq [x (range n_notes)]
                                        (def note_index (nth new_vector x))
                                        (def ti1 (+ x            (+ 3 beat_)))
                                        (def ti2 (+ x  (+ legato (+ 3 beat_))))
