...
(note :bb4) (note :bb5)
                (note :gb4) (note :gb5) (note :db4) (note :db5)
                  ])
(println all_notes)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(def b12 (atom 1))
(def b1 (atom [(note :a1) (note :a1) (note :e2) (note :e2) (note :e2) ]))
(def b2 (atom [(note :e2) (note :e2) (note :e2) (note :a1) (note :a1) ]))
           ...
           (if (= end_level new-state) (do
             ...
             (if (not= end_level new-state) (do
                                   (def last_buff (vector (nth all_notes (dec @level)))) ; index start at 0
                                      ...
                                      (def bass_vector (into [] (take n_notes @b1)))
                                      (if (= @b12 2) (do
                                                       (reset! b12 1)
                                                       (def bass_vector (into [] (take n_notes @b2)))
                                                      )
                                                      (swap! b12 inc)
                                       )
                                      ...
                                      (doseq [x (range n_notes)]
                                        ...
                                        (def note_bass (nth bass_vector x))
                                        (def ti1 (+ x            (+ 1 beat_)))
                                        ...
                                        (apply-at (metro ti1)    smi2bass [note_bass])
                                        (apply-at (metro ti2) smi2off [note_bass])
                                        )
